<?php

/**
 * @link              http://nishantshaigram.github.io
 * @since             1.0.0
 * @package           DContact
 *
 * @wordpress-plugin
 * Plugin Name:       DContact
 * Plugin URI:        http://nishantshaigram.github.io
 * Description:       Simple Contact Form
 * Version:           1.0.0
 * Author:            Nishant Shaligram
 * Author URI:        http://nishantshaigram.github.io
 * License:           GPL-3.0+
 * License URI:       http://www.gnu.org/licenses/gpl-3.0.txt
 * Text Domain:       dcontact
 * Domain Path:       /languages
 */

// If this file is called directly, abort.
if ( ! defined( 'WPINC' ) ) {
	die;
}

/**
 * Currently plugin version.
 * Start at version 1.0.0 and use SemVer - https://semver.org
 * Rename this for your plugin and update it as you release new versions.
 */
define( 'DCONTACT_VERSION', '1.0.0' );

/**
 * The code that runs during plugin activation.
 * This action is documented in includes/class-plugin-name-activator.php
 */
function activate_dcontact() {
	require_once plugin_dir_path( __FILE__ ) . 'includes/class-dcontact-activator.php';
	DContact_Activator::activate();
}

/**
 * The code that runs during plugin deactivation.
 * This action is documented in includes/class-plugin-name-deactivator.php
 */
function deactivate_dcontact() {
	require_once plugin_dir_path( __FILE__ ) . 'includes/class-dcontact-deactivator.php';
	DContact_Deactivator::deactivate();
}

register_activation_hook( __FILE__, 'activate_dcontact' );
register_deactivation_hook( __FILE__, 'deactivate_dcontact' );

/**
 * The core plugin class that is used to define internationalization,
 * admin-specific hooks, and public-facing site hooks.
 */
require plugin_dir_path( __FILE__ ) . 'includes/class-dcontact.php';

/**
 * Begins execution of the plugin.
 *
 * Since everything within the plugin is registered via hooks,
 * then kicking off the plugin from this point in the file does
 * not affect the page life cycle.
 *
 * @since    1.0.0
 */
function run_dcontact() {

	$plugin = new DContact();
	$plugin->run();

}
run_dcontact();
